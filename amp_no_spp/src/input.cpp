...
static const uint32_t ENC_SW_LONG_PRESS_MS = 1000;  // поріг утримування

// Час натискання (для відліку утримування)
static uint32_t encSwPressTime    = 0;
static bool     encSwLongFired    = false;  // щоб не повторювати подію
...
InputEvent inputPollEncoder() {
    ...
    if (encSwPressed) {
        // Утримування досягло порогу — генеруємо LONG одноразово
        if (!encSwLongFired && (millis() - encSwPressTime >= ENC_SW_LONG_PRESS_MS)) {
            encSwLongFired = true;
            event.type = EVENT_ENCODER_LONG;
            event.value = 1;
            DEBUG_PRINTLN("[ENC] Long press");
            return event;
        }
    } else if (!encSwLongFired && encSwPressTime > 0) {
        // Кнопка відпущена без тривалого утримування — короткий клік
        encSwPressTime = 0;
...
void handleInputEvent(InputEvent event) {
    switch (event.type) {
        ...
        case EVENT_ENCODER_LONG: {
            ...
            Serial.println(muted ? "[Encoder] Mute увімкнено" : "[Encoder] Mute вимкнено");
            break;
        }
